...
from network.models import Question
...
def collegegallery(request):
    return render(request, "network/collegegallery.html")
...
def ask(request):
    if request.method == "POST":
        ...
        askme_q = request.POST.get('askme_q')
        contactme = Question(name = name,askme_q=askme_q, date = datetime.today())# creating object of contact
        contactme.save()
    return render(request, "network/ask.html")

def senior(request):
    return render(request, "network/senior.html")